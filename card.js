    ...
    export let RozgonyiCard = new tanarCards("Rozgonyi","vérszomjas","tövisesbőr","summon","EmptyAbility", "Amíg a pályán van, <b>hátsó</b> sorod összes kártyája <b>+1atk</b>. <b>Elkárhozáskor</b> minden kártyádnak <b>+1atk</b> és <b>+1hp</b>.", 2, 10, Rozgonyi, 3, 5, "character", RozgonyiGacha, "RozgonyiCard", RoziA, "Szeret sütni. Szeret még legózni és gombócot enni is. A fiát láthatólag annyira nem.", []);
    export let TabiCard = new tanarCards("Tábi", "vérszomjas","fürge támadás","summon","EmptyAbility", "Minden <b>'tunya'</b> kártya <b>+2atk</b>.", 1, 4, Tabi, 3, 2, "character", TabiGacha, "TabiCard", TabiA, "Tesitanár és röplabdaedző. Epic gamer szabadidejében. Sose hagyja ki az alkalmat, hogy politizáljon.", []);
    ...
    export let KutiCard = new tanarCards("Kúti","veszett","kettős támadás","summon","EmptyAbility", "Egy karakterkártyád életerőpontjai megduplázódnak.", 3, 2, Kuti, 5, 3, "character", KutiGacha, "KutiCard", KutiA, "Irodalom, nyelvtan, matematika, tesi szakos, síoktató, asztronauta és agysebész.", ["kettős:0"]);
    ...
    export let MartiCard = new tanarCards("Fehér Márta","veszett","életelszívás_1","summon","EmptyAbility", "Amíg a pályán van, minden sebzés, ami a kártyáid felé irányulnak 1-gyel csökken.", 1, 1, Marti, 4, 1, "character", MartiGacha, "MartiCard", MartiA, "Az ing, nem azing. Hetest kér.", []);
    export let MeszarosCard = new tanarCards("Mészaros","vérszomjas","","summon","EmptyAbility", "Húzd fel a legfelső kártyát a paklidból!", 2, 5, Meszaros, 3, 3, "character", MeszarosGacha, "MeszarosCard", MeszarosA, "Médiatanár. A diákok gyakran jobban tudják az anyagot.", []);
    export let JeffCard = new tanarCards("Jeff","lelkiismeretes","","summon","EmptyAbility", "", 5, 5, Jeff, 6, 3, "character", JeffGacha, "JeffCard", JeffA, "Plays golf with you on the weekends. Loves his wife, kids and family dog.", []);
    ...
    export let FiloRekaCard = new tanarCards("Filó Réka","tunya","","summon","EmptyAbility", "Ha megsebzik, de nem kárhozik el, +1spellmana.", 6, 8, FiloReka, 3, 5, "character", GachaPH, "FiloRekaCard",FiloA, "Sajnos családi életét az érettségiben nem kérdezik.", []);
    export let IzigCard = new tanarCards("Izing Máté","tunya","kettős támadás","summon","EmptyAbility", "Egy karakterkártyát visszaküld a tábláról a játékos kezébe.", 3, 5, Izig, 4, 4, "character", GachaPH, "IzigCard", IzigA, "Szeret utazni. Báló Dávidot is szereti.", ["kettős:0"]);
    ...
    export let TothKareszCard = new tanarCards("Karesz","vérszomjas","","summon","EmptyAbility", "", 6, 10, TothKaresz, 3, 6, "character", GachaPH, "TothKareszCard", KareszA, "Gurulási súrlódási együtthatója 0,7.", []);
    export let VeronikaCard = new tanarCards("Németh Veronika","lelkiismeretes","tövisesbőr","summon","EmptyAbility", "A szomszédjai támadóereje ennek a kártyának a támadóerejének a 30%-ával nő lefelé kerekítve.", 4, 9, Veronika, 3, 5, "character", GachaPH, "VeronikaCard", VeronikaA, "Vigyorog, mint a tejbetök. Nagyon cuki.", []);
    export let WeiszCard = new tanarCards("Weisz","tunya","kettős támadás","summon","EmptyAbility","Húzd fel a legdrágább kártyát a paklidból!", 2, 5, Weisz, 4, 3, "character", GachaPH, "WeiszCard", WeiszA, "Teljességgel kiszámíthatatlan. Támogatja, hogy órán Cards Against Humanity-t játsszunk.", ["kettős:0"]);
    export let TritzCard = new tanarCards("Tritzy","vérszomjas","","summon","EmptyAbility", "A paklid 2 legalsó karakterkártyája a temedőbe kerül. ", 3, 5, Tritz, 4, 3, "character", GachaPH, "TritzCard", TritzyA, "😄   -->   😈", []);
    export let AnderleneCard = new tanarCards("Anderléné","vérszomjas","életelszívás_2","summon","EmptyAbility", "Húzz fel egy karakterkártyát a temetőből. A felhúzott kártya 1-gyel kevesebb manába kerül az eredeti értékénél.", 2, 3, Anderlene, 4, 4, "character", GachaPH, "AnderleneCard", gachaPHAudio, "A nyelvtanárok nyelvtanára.", []);
    export let SagineCard = new tanarCards("Ságiné","veszett","","summon","EmptyAbility", "Ha van a húzópaklidban biosszertár csontváza kártya, húzd fel a kezedbe!", 2, 3, Sagine, 3, 2, "character", GachaPH, "SagineCard", gachaPHAudio, "Szeretni máshogy is lehet. Nem kell bántani egymást.", []);

    //DIAKOK
    export let ArhoCard = new diakCards("Arho","tunya","","summon","EmptyAbility", "Rakj le egy ingyen követ a táblára a temető lélekszámával megegyező életerőponttal.", 5, 5, Arho, 4, 3, "character", ArhoGacha, "ArhoCard", ArhoA, "7 évesen megfejtette a tudományt. Hisz a szabad akaratban.", []); 
    export let BarniCard = new diakCards("Barni","veszett","fürge támadás","summon","EmptyAbility", "Amíg eltakarja egy kő, 'vérszomjas'.", 4, 4, Barni, 3, 4, "character", BarniGacha, "BarniCard", BarniA, "Szeretik a lányok, szeretik a fiúk, ő pedig a krumplis tésztát szereti.", []);
    export let EszterCard = new diakCards("Eszter","vérszomjas","","summon","EmptyAbility", "Visszakerül a kezedbe, támadóerejét megtartva. Végtelenszer megismételhető.", 1, 1, Eszter, 5, 1, "character", EszterGacha, "EszterCard", EszterA, "Nem tart otthon tyúkokat, de nagyon szeretne. Este 9-kor kizavarják a kollégium gépterméből.", []);
    export let OliviaCard = new diakCards("Olívia","lelkiismeretes","kihívó","summon","EmptyAbility", "Amíg játékban van, a tábla összes kártyája 'veszetté' és 'vérszomjassá' válik.", 2, 6, Olivia, 4, 3, "character", OliviaGacha, "OliviaCard", OliviaA, "Lehelt már számtalanszon visszautasította. Oliver ikertestvére. Lehel ezt még nem tudja.", []);
